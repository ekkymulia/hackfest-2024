...
const ProjectDetail = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setStatus(responseData.results.data.status);
  ...
  const updateData = async () => {
    try {
      ...
      const responseData = await res.json();
      console.log(responseData.results.projects);
      setData(responseData.results.projects);
      setLoading(false);
    } catch (err) {
      console.error("Fetch project error:", err);
      setError("Error fetching project data");
      ...
      {data ? (
        <section className="p-8 auto-cols-max	 min-w-[50%] flex flex-row">
          <div className="col-span-2 ">
                ...
                <div className="flex flex-col">
                  {userData.role === "1" && (
                    <>
                      ...
                      >
                        {(ps) => (
                          <SelectItem key={ps.value}>{ps.label}</SelectItem>
                        )}
                      </Select>
                    </>
                  )}
              ...
              <div className="col-span-4 flex flex-col">
                ...
                {isEditing ? (
                  <Input
                    ...
                    type="date"
                    className="col-span-6 max-w-md"
                    value={ProjectForm.wanted_deadline}
                    name="wanted_deadline"
                    onChange={handleInputChange}
                  />
                ) : (
                  <span>
                    {data.wanted_deadline ? data.wanted_deadline : "-"}
                  </span>
                )}
              </div>
              <div className="col-span-4 flex flex-col">
                ...
                {isEditing ? (
                  <Input
                    ...
                    type="date"
                    className="col-span-6 max-w-md"
                    value={ProjectForm.target_deadline}
                    name="target_deadline"
                    onChange={handleInputChange}
                  />
                ) : (
                  <span>
                    {data.target_deadline ? data.target_deadline : "-"}
                  </span>
                )}
              </div>
              <div className="col-span-4 flex flex-col">
                <span>Freelancers Needed:</span>
                {isEditing && userData.role === "1" ? (
                  <Input
                    size={"md"}
                    type="number"
                    className="col-span-6 max-w-md"
                    value={ProjectForm.asignee_needed}
                    name="asignee_needed"
                    onChange={handleInputChange}
                  />
                ) : (
            ...
            <div className="">
              <div className="flex flex-col">
                <Tabs aria-label="Options" className="col-span-6">
          ...
          <div className="col-span-1 mx-5 px-5 min-w-[50%]">
            <div className="lg:pl-5">
              <span className="font-bold text-lg my-4">
                Assignees and Sprints Board
              </span>

              <div className="flex flex-col">
                <Tabs aria-label="Options" className="col-span-12 mt-1">
                  <Tab key="assignee" title="Assignees">
                    ...
                    {userData.role == 3 &&
                    !data.asignee_uid.includes(userData.id) ? (
